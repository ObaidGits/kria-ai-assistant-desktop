...
fn search_package_schema_keeps_query_and_alias_name() {
    ...
    let def = reg.get_def("search_package").expect("search_package should be registered");

    let query_param = def.parameters.iter().find(|p| p.name == "query").expect("query param should exist");
    assert!(query_param.required, "query should remain required for canonical calls");

    let alias_param = def.parameters.iter().find(|p| p.name == "name").expect("name alias should exist");
...
async fn search_package_accepts_name_alias_when_query_missing() {
    ...
    let reg = build_default_registry();
    let handler = reg.get_handler("search_package").expect("search_package handler should exist");
    ...
    assert!(err.contains("unknown package source"), "expected source validation error, got: {err}");
    assert!(!err.contains("query parameter is required"), "name alias should satisfy query input, got: {err}");
}

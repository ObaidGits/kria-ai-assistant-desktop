        ...
        match run_git(&["status", "--porcelain", "-b"], Some(path)).await {
            Ok(output) => {
                ...
                let branch = lines
                    ...
                    .unwrap_or_default();
                let changes: Vec<serde_json::Value> = lines
                    .iter()
                    .skip(1)
                    .filter(|l| !l.is_empty())
                    .map(|l| {
...
struct GitCommit;
#[async_trait]
impl ToolHandler for GitCommit {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let path = params["path"].as_str().unwrap_or(".");
        ...
        match run_git(&["commit", "-m", message], Some(path)).await {
            Ok(output) => {
                ToolResult::ok(serde_json::json!({ "committed": true, "output": output.trim() }))
            }
            Err(e) => ToolResult::err(e),
        }
    }
...
#[async_trait]
impl ToolHandler for GitBranchList {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let path = params["path"].as_str().unwrap_or(".");
...
struct GitCheckout;
#[async_trait]
impl ToolHandler for GitCheckout {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let path = params["path"].as_str().unwrap_or(".");
        ...
        match run_git(&args, Some(path)).await {
            Ok(output) => {
                ToolResult::ok(serde_json::json!({ "branch": branch, "output": output.trim() }))
            }
            Err(e) => ToolResult::err(e),
        }
    }
    ...
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        ...
        match run_git(&args, Some(path)).await {
            Ok(output) => ToolResult::ok(
                serde_json::json!({ "action": if pop {"pop"} else {"stash"}, "output": output.trim() }),
            ),
            Err(e) => ToolResult::err(e),
        }
    }
...
#[async_trait]
impl ToolHandler for AnalyzeProject {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let path = params["path"].as_str().unwrap_or(".");
        ...
        let lang_summary: Vec<serde_json::Value> = lang_vec
            .iter()
            .take(10)
        ...
        if root.join("tauri.conf.json").exists() {
            framework = "Tauri + Rust".into();
        } else if root.join("next.config.js").exists() || root.join("next.config.ts").exists() {
            framework = "Next.js".into();
        } else if root.join("vite.config.ts").exists() || root.join("vite.config.js").exists() {
            framework = format!("{} + Vite", framework);
        } else if root.join("django").exists() || root.join("manage.py").exists() {
            framework = "Django".into();
        } else if root.join("Gemfile").exists() {
            framework = "Ruby on Rails".into();
        }

        // Git info
        let git_branch = run_git(&["branch", "--show-current"], Some(path))
            .await
            .ok()
...
struct QuerySqlite;
#[async_trait]
impl ToolHandler for QuerySqlite {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
...
#[async_trait]
impl ToolHandler for DescribeDatabase {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let db_path = params["db_path"].as_str().unwrap_or("");
        if db_path.is_empty() {
    ...
    let tools: Vec<(ToolDef, Arc<dyn ToolHandler>)> = vec![
        ...
        (
            ToolDef {
                ...
                description: "Show git status: current branch and changed files".into(),
        ...
        (
            ToolDef {
                name: "git_diff".into(),
                description: "Show git diff for working directory or a specific file".into(),
                category: "developer".into(),
                default_tier: RiskLevel::Green,
                min_tier: "standard",
                parameters: vec![
            ...
            ToolDef {
                ...
                parameters: vec![
                    ...
                    param(
                        "all",
                        "boolean",
                        "Stage all changes before committing (default: true)",
                        false,
                    ),
                ],
            },
        ...
        (
            ToolDef {
                ...
                description: "Switch to a branch or create a new branch".into(),
                category: "developer".into(),
        ...
        (
            ToolDef {
                name: "git_stash".into(),
                description: "Stash or pop stashed changes".into(),
                category: "developer".into(),
                default_tier: RiskLevel::Yellow,
                min_tier: "standard",
                parameters: vec![
                    ...
                    param(
                        "pop",
                        "boolean",
                        "Pop the stash instead of creating one",
                        false,
                    ),
                ],
            },
        ...
        (
            ToolDef {
                ...
                description:
                    "Analyze a project: detect languages, framework, file counts, and structure"
                        .into(),
                category: "developer".into(),
                default_tier: RiskLevel::Green,
                min_tier: "standard",
        ...
        (
            ToolDef {
                name: "diff_files_unified".into(),
                description: "Show unified diff between two files using system diff".into(),
                category: "developer".into(),
                default_tier: RiskLevel::Green,
                min_tier: "standard",
                parameters: vec![
        ...
        (
            ToolDef {
                name: "query_sqlite".into(),
                description: "Run a read-only SQL query on a SQLite database".into(),
                category: "developer".into(),
                default_tier: RiskLevel::Green,
                min_tier: "standard",
                parameters: vec![
                    ...
                    param(
                        "sql",
                        "string",
                        "SQL query (SELECT/PRAGMA/EXPLAIN only)",
                        true,
                    ),
                ],
            },
        ...
        (
            ToolDef {
                name: "describe_database".into(),
                description: "List all tables and views in a SQLite database".into(),
                category: "developer".into(),
                default_tier: RiskLevel::Green,
                min_tier: "standard",
                parameters: vec![param(

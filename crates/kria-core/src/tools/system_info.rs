...
struct GetCpuUsage;
#[async_trait]
impl ToolHandler for GetCpuUsage {
...
#[async_trait]
impl ToolHandler for GetMemoryInfo {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        let mut sys = sysinfo::System::new();
        sys.refresh_memory();
...
struct GetDiskSpace;
#[async_trait]
impl ToolHandler for GetDiskSpace {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        let disks = sysinfo::Disks::new_with_refreshed_list();
        let info: Vec<serde_json::Value> = disks
            .iter()
            .map(|d| {
                serde_json::json!({
...
#[async_trait]
impl ToolHandler for GetNetworkStatus {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        let networks = sysinfo::Networks::new_with_refreshed_list();
        let info: Vec<serde_json::Value> = networks
            .iter()
            .map(|(name, data)| {
                serde_json::json!({
                    "interface": name,
...
#[async_trait]
impl ToolHandler for GetBatteryStatus {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
...
struct GetGpuInfo;
#[async_trait]
impl ToolHandler for GetGpuInfo {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
            ...
            .await
        {
            ...
            _ => ToolResult::ok(
                serde_json::json!({ "gpu": "not detected or nvidia-smi not available" }),
            ),
        }
    }
}
...
#[async_trait]
impl ToolHandler for GetSystemUptime {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {

...
#[async_trait] impl ToolHandler for OpenApplication {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        ...
        let result = if cfg!(target_os = "linux") {
            ...
            tokio::process::Command::new("cmd").args(["/C", "start", "", app]).args(&args).spawn()
        } else {
            tokio::process::Command::new("open").arg("-a").arg(app).args(&args).spawn()
        };
...
#[async_trait] impl ToolHandler for ListRunningApps {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        let mut sys = sysinfo::System::new();
        sys.refresh_processes(sysinfo::ProcessesToUpdate::All, true);
        let procs: Vec<serde_json::Value> = sys.processes().iter()
            .filter(|(_, p)| !p.name().to_string_lossy().is_empty())
            .map(|(pid, p)| serde_json::json!({
                "pid": pid.as_u32(),
...
struct FocusWindow;
#[async_trait] impl ToolHandler for FocusWindow {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        ...
        if cfg!(target_os = "linux") {
            let output = tokio::process::Command::new("wmctrl")
                ...
                .output().await;
            match output {
                Ok(o) if o.status.success() => ToolResult::ok(serde_json::json!({ "focused": title })),
                _ => ToolResult::err(format!("could not focus window '{title}' (wmctrl required)"))
            }
        } else {
...
#[async_trait] impl ToolHandler for CloseApplication {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        ...
        for (_pid, proc_) in sys.processes() {
            if proc_.name().to_string_lossy().to_lowercase().contains(&name.to_lowercase()) {
                proc_.kill();
                killed += 1;
            }
        }
...
struct KillProcess;
#[async_trait] impl ToolHandler for KillProcess {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
...
pub fn register(reg: &ToolRegistry) {
    let tools: Vec<(ToolDef, Arc<dyn ToolHandler>)> = vec![
        (ToolDef {
            ...
            category: "app_lifecycle".into(), default_tier: RiskLevel::Green, min_tier: "lite",
            parameters: vec![
        ...
        (ToolDef {
            name: "list_running_apps".into(), description: "List all running processes with CPU and memory usage".into(),
            category: "app_lifecycle".into(), default_tier: RiskLevel::Green, min_tier: "lite",
            parameters: vec![],
        }, Arc::new(ListRunningApps)),
        (ToolDef {
            name: "focus_window".into(), description: "Bring a window to the foreground by title".into(),
            category: "app_lifecycle".into(), default_tier: RiskLevel::Green, min_tier: "standard",
            parameters: vec![param("title", "string", "Window title (partial match)", true)],
        }, Arc::new(FocusWindow)),
        (ToolDef {
            name: "close_application".into(), description: "Close an application by name".into(),
            category: "app_lifecycle".into(), default_tier: RiskLevel::Yellow, min_tier: "lite",
            parameters: vec![param("name", "string", "Application name", true)],
        }, Arc::new(CloseApplication)),
        (ToolDef {
            name: "kill_process".into(), description: "Kill a process by PID".into(),
            category: "app_lifecycle".into(), default_tier: RiskLevel::Yellow, min_tier: "lite",

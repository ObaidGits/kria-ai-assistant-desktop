...
#[async_trait] impl ToolHandler for GetActiveWindow {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        if cfg!(target_os = "linux") {
            ...
            match output {
                ...
                _ => ToolResult::err("xdotool not available — install it with: sudo apt install xdotool")
            }
        } else {
...
struct MoveWindow;
#[async_trait] impl ToolHandler for MoveWindow {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        ...
        if cfg!(target_os = "linux") {
            let result = tokio::process::Command::new("wmctrl")
                ...
                .output().await;
            match result {
                Ok(o) if o.status.success() => ToolResult::ok(serde_json::json!({ "moved": title, "x": x, "y": y })),
...
struct ResizeWindow;
#[async_trait] impl ToolHandler for ResizeWindow {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let title = params["title"].as_str().unwrap_or("");
        ...
        if cfg!(target_os = "linux") {
            let result = tokio::process::Command::new("wmctrl")
                ...
                .output().await;
            match result {
                Ok(o) if o.status.success() => ToolResult::ok(serde_json::json!({ "resized": title, "width": w, "height": h })),
                _ => ToolResult::err(format!("failed to resize window '{title}' (wmctrl required)"))
            }
        } else {
...
#[async_trait] impl ToolHandler for MaximizeWindow {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let title = params["title"].as_str().unwrap_or("");
        if cfg!(target_os = "linux") {
            let result = tokio::process::Command::new("wmctrl")
                ...
                .output().await;
            match result {
                Ok(o) if o.status.success() => ToolResult::ok(serde_json::json!({ "maximized": title })),
...
#[async_trait] impl ToolHandler for MinimizeWindow {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let title = params["title"].as_str().unwrap_or("");
        if cfg!(target_os = "linux") {
            let result = tokio::process::Command::new("xdotool")
                ...
                .output().await;
            match result {
                Ok(o) if o.status.success() => ToolResult::ok(serde_json::json!({ "minimized": title })),
...
struct TileWindows;
#[async_trait] impl ToolHandler for TileWindows {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let layout = params["layout"].as_str().unwrap_or("side-by-side");
        let windows: Vec<String> = params["windows"].as_array()
            .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
            .unwrap_or_default();

        if cfg!(target_os = "linux") {
            // Get screen dimensions
            let xdp = tokio::process::Command::new("xdpyinfo")
                .output().await;
            let (sw, sh) = xdp.ok()
                .filter(|o| o.status.success())
                .and_then(|o| {
                    ...
                    text.lines()
                        ...
                        .and_then(|l| {
                            ...
                            parts.get(1).and_then(|dim| {
                                ...
                                Some((xy.first()?.parse::<i64>().ok()?, xy.get(1)?.parse::<i64>().ok()?))
                            })
                        })
                })
                .unwrap_or((1920, 1080));
            ...
            match layout {
                "side-by-side" => {
                    ...
                    let _ = tokio::process::Command::new("wmctrl")
                        .args(["-r", &windows[0], "-b", "remove,maximized_vert,maximized_horz"])
                        .output().await;
                    let _ = tokio::process::Command::new("wmctrl")
                    ...
                    // Right half
                    let _ = tokio::process::Command::new("wmctrl")
                        .args(["-r", &windows[1], "-b", "remove,maximized_vert,maximized_horz"])
                        .output().await;
                    let _ = tokio::process::Command::new("wmctrl")
                        .args(["-r", &windows[1], "-e", &format!("0,{half_w},0,{half_w},{sh}")])
                        .output().await;
                    ToolResult::ok(serde_json::json!({ "layout": "side-by-side", "windows": windows }))
                }
                "grid" => {
                    let half_w = sw / 2;
                    let half_h = sh / 2;
                ...
                _ => ToolResult::err(format!("unknown layout '{layout}'. Supported: side-by-side, grid"))
            }
        } else {
...
struct OpenUrl;
#[async_trait] impl ToolHandler for OpenUrl {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
...
struct ListWindows;
#[async_trait] impl ToolHandler for ListWindows {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        if cfg!(target_os = "linux") {
            ...
            match output {
                Ok(o) if o.status.success() => {
                    ...
                    let windows: Vec<serde_json::Value> = text.lines()
                        .filter(|l| !l.is_empty())
                        .map(|line| {
                        ...
                        .collect();
                    ToolResult::ok(serde_json::json!({ "windows": windows, "count": windows.len() }))
                }
                _ => ToolResult::err("wmctrl not available — install it with: sudo apt install wmctrl")
            }
        } else {
...
pub fn register(reg: &mut ToolRegistry) {
    let tools: Vec<(ToolDef, Arc<dyn ToolHandler>)> = vec![
        (ToolDef {
            ...
            description: "Get the currently focused window title, PID, and application class".into(),
            category: "desktop".into(), default_tier: RiskLevel::Green, min_tier: "standard",
            parameters: vec![],
        ...
        (ToolDef {
            ...
            category: "desktop".into(), default_tier: RiskLevel::Yellow, min_tier: "standard",
            parameters: vec![
        ...
        (ToolDef {
            ...
            category: "desktop".into(), default_tier: RiskLevel::Yellow, min_tier: "standard",
            parameters: vec![param("title", "string", "Window title (partial match)", true)],
        ...
        (ToolDef {
            ...
            parameters: vec![
                ...
                param("layout", "string", "Layout: 'side-by-side' or 'grid'", false),
            ],
        }, Arc::new(TileWindows)),
        (ToolDef {
            name: "open_url".into(),
            description: "Open a URL in the default web browser".into(),
            category: "desktop".into(), default_tier: RiskLevel::Green, min_tier: "lite",
            parameters: vec![param("url", "string", "URL to open (must start with http:// or https://)", true)],
        }, Arc::new(OpenUrl)),
    ];

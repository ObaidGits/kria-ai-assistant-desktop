...
#[async_trait] impl ToolHandler for ParseDocument {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        ...
        if matches!(ext.as_str(), "pdf" | "docx" | "xlsx") {
            if let Some(result) = self.sidecar.try_extract(path, &["text", "tables", "sections"]).await {
...
#[async_trait] impl ToolHandler for ParseCsv {
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        let path = params["path"].as_str().unwrap_or("");
        ...
        match tokio::fs::read_to_string(path).await {
            Ok(content) => {
                ...
                let sample_rows: Vec<Vec<&str>> = lines.iter()
                    .skip(1)
                    .take(max_rows)
    ...
    async fn execute(&self, params: serde_json::Value) -> ToolResult {
        ...
        if matches!(ext.as_str(), "pdf" | "docx") {
            if let Some(result) = self.sidecar.try_extract(path, &["text", "sections"]).await {
                ...
                let sections = result.get("sections").cloned().unwrap_or(serde_json::json!([]));
...
pub fn register_with_sidecar(reg: &mut ToolRegistry, sidecar: Option<Arc<SidecarBridge>>) {
    ...
    let tools: Vec<(ToolDef, Arc<dyn ToolHandler>)> = vec![
        (ToolDef {
            name: "parse_document".into(), description: "Extract text from any document (PDF, DOCX, CSV, TXT, MD, JSON, YAML, etc.)".into(),
            category: "documents".into(), default_tier: RiskLevel::Green, min_tier: "lite",
            parameters: vec![
                param("path", "string", "Document file path", true),
            ],
        }, Arc::new(ParseDocument { sidecar: doc_sc.clone() })),
        (ToolDef {
            name: "parse_csv".into(), description: "Parse CSV file with column detection and sample rows".into(),
            category: "documents".into(), default_tier: RiskLevel::Green, min_tier: "lite",
            parameters: vec![
                param("path", "string", "CSV file path", true),
                param("max_rows", "integer", "Max sample rows (default 100)", false),
            ],
        }, Arc::new(ParseCsv { sidecar: doc_sc.clone() })),
        (ToolDef {
            name: "summarize_document".into(), description: "Get document statistics, sections, and preview".into(),
            category: "documents".into(), default_tier: RiskLevel::Green, min_tier: "lite",

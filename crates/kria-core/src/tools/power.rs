...
struct Sleep;
#[async_trait] impl ToolHandler for Sleep {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        ...
        match tokio::process::Command::new("sh").args(["-c", cmd]).status().await {
...
struct Hibernate;
#[async_trait] impl ToolHandler for Hibernate {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        let cmd = if cfg!(target_os = "linux") {
            "systemctl hibernate"
        } else {
...
#[async_trait] impl ToolHandler for ShutdownSystem {
...
struct RebootSystem;
#[async_trait] impl ToolHandler for RebootSystem {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        let cmd = if cfg!(target_os = "linux") { "reboot" } else { "shutdown /r /t 0" };
        match tokio::process::Command::new("sh").args(["-c", cmd]).status().await {
...
struct LockScreen;
#[async_trait] impl ToolHandler for LockScreen {
    async fn execute(&self, _params: serde_json::Value) -> ToolResult {
        let cmd = if cfg!(target_os = "linux") {
...
pub fn register(reg: &ToolRegistry) {
    let tools: Vec<(ToolDef, Arc<dyn ToolHandler>)> = vec![
        // GREEN
        (ToolDef {
            name: "lock_screen".into(), description: "Lock the screen".into(),
            category: "power".into(), default_tier: RiskLevel::Green, min_tier: "lite",
            parameters: vec![],
        }, Arc::new(LockScreen)),
        // YELLOW
        (ToolDef {
            name: "sleep".into(), description: "Put system to sleep".into(),
            category: "power".into(), default_tier: RiskLevel::Yellow, min_tier: "lite",
            parameters: vec![],
        }, Arc::new(Sleep)),
        (ToolDef {
            name: "hibernate".into(), description: "Hibernate the system".into(),
            category: "power".into(), default_tier: RiskLevel::Yellow, min_tier: "lite",
            parameters: vec![],
        }, Arc::new(Hibernate)),
        // RED
